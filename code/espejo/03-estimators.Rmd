...
las_vegas_2 <- merge(las_vegas, data.frame(review_bins), by="review_count")
```
...
las_vegas_2 <- merge(las_vegas_2, data.frame(age_bins), by="age")
```

## Simple substitution estimator.
I used R Lab 2.
```{r}
e_a1 <- las_vegas_2 %>% filter(star_intervention==1) %>%
  group_by(neighborhood, american, chain) %>%
  summarize(e.ya1=mean(open_2019))
# nrow(e_a1)

e_a0 <- las_vegas_2 %>% filter(star_intervention==0) %>%
  group_by(neighborhood, american, chain=) %>%
...
probs <- las_vegas_2 %>% 
  group_by(neighborhood, american, chain) %>% 
  summarize(p=n()/nrow(las_vegas_2))
...
```{r}
library(nnet)
gAW_reg  <- multinom(star_intervention~neighborhood+american+review_count+age, data=las_vegas_2)
gAW_pred <- predict(gAW_reg, type="probs")

gAW <- as.numeric(gAW_pred)
wt  <- gAW^-1

IPTW <- mean(wt*as.numeric(las_vegas_2$star_intervention==1)*las_vegas_2$open_2019) -
        mean(wt*as.numeric(las_vegas_2$star_intervention==0)*las_vegas_2$open_2019)
IPTW
```

```{r}
IPTW_or <- mean(wt*as.numeric(las_vegas_2$star_intervention==1)*las_vegas_2$open_2019) / mean( wt*as.numeric(las_vegas_2$star_intervention==1)) - mean(wt*as.numeric(las_vegas_2$star_intervention==0)*las_vegas_2$open_2019) / mean( wt*as.numeric(las_vegas_2$star_intervention==0))
IPTW_or
```